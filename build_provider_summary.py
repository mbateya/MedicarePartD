...
GROUP_COLS = ["Year", "Generic Name", "Brand Name", "Prescriber Name"]
METRIC_COLS = [
    "Total Claims",
    "Total 30-Day Fills",
    "Total Days Supply",
...
for i in range(n_groups):
    ...
    if (i + 1) % 10 == 0 or i == n_groups - 1:
        ...
        # Merge intermediate results to keep memory flat
...
# Keep top N providers per (Year, Generic Name) by Total Drug Cost
top = (
    summary.sort_values("Total Drug Cost", ascending=False)
    .groupby(["Year", "Generic Name"], group_keys=False)
    .head(TOP_N_PER_DRUG_YEAR)
    .reset_index(drop=True)
)

# Derived metrics
top["Cost per Claim"] = (
    top["Total Drug Cost"] / top["Total Claims"].replace(0, pd.NA)
)
top["Cost per 30-Day Fill"] = (
    top["Total Drug Cost"] / top["Total 30-Day Fills"].replace(0, pd.NA)
)
